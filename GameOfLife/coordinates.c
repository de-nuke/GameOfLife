...
{	
	...
	size = (new_coords->height * new_coords->width)/4;
	const_size = size;
	printf("%d \n",size);
	...
	while(fscanf(in, "%d %d", new_coords->x+i, new_coords->y+i) != EOF)
	{
		if(new_coords->n > new_coords->width*new_coords->height) {
			printf("%d ", size);
			return new_coords; }
		if(new_coords->n > size)
			{
				nextsize = ((size+const_size)*sizeof(int));
				if((xx = realloc(new_coords->x, 1)) == NULL)
					return NULL;
				if((yy = realloc(new_coords->y, 1)) == NULL)
					return NULL;
				size+=const_size;
				new_coords->x = xx;
				new_coords->y = yy;
